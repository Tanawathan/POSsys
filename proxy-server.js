...
// KDS 專用端點 - 獲取準備中和烹飪中的訂單
app.get('/api/kds/orders', async (req, res) => {
    try {
        console.log('🍽️ KDS: 獲取待處理訂單...');
        ...
        const response = await fetch(`${NOTION_BASE_URL}/databases/${ORDERS_DATABASE_ID}/query`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${NOTION_API_KEY}`,
                'Content-Type': 'application/json',
        ...
        if (response.ok) {
            const data = await response.json();
            console.log(`✅ KDS: 找到 ${data.results?.length || 0} 筆待處理訂單`);
            res.json(data);
        } else {
            const error = await response.json();
            console.error(`❌ KDS: 查詢失敗`, error);
            res.status(response.status).json(error);
        }
    } catch (error) {
        console.error('❌ KDS: 端點錯誤:', error);
        res.status(500).json({ 
            error: 'KDS 查詢錯誤', 
            message: error.message 
        });
...
// KDS 專用端點 - 更新訂單狀態
app.patch('/api/kds/orders/:orderId/status', async (req, res) => {
    try {
        const { orderId } = req.params;
        const { status } = req.body;
        
        console.log(`🔄 KDS: 更新訂單 ${orderId} 狀態為 ${status}`);
        ...
        const response = await fetch(`${NOTION_BASE_URL}/pages/${orderId}`, {
            method: 'PATCH',
            headers: {
                'Authorization': `Bearer ${NOTION_API_KEY}`,
                'Content-Type': 'application/json',
        ...
        if (response.ok) {
            const data = await response.json();
            console.log(`✅ KDS: 訂單 ${orderId} 狀態更新成功`);
            res.json(data);
        } else {
            const error = await response.json();
            console.error(`❌ KDS: 狀態更新失敗`, error);
            res.status(response.status).json(error);
        }
    } catch (error) {
        console.error('❌ KDS: 狀態更新錯誤:', error);
        res.status(500).json({ 
            error: 'KDS 狀態更新錯誤', 
            message: error.message 
        });
...
// 更新桌況狀態（結帳用）
app.post('/api/notion/update-table-status', async (req, res) => {
    try {
        const { tableNumber, status } = req.body;
        console.log(`🔄 更新桌況狀態 - 桌號: ${tableNumber}, 狀態: ${status}`);

        // 查詢桌號對應的 Notion 記錄
        const tablesDatabase = await notion.databases.query({
        ...
        if (tablesDatabase.results.length === 0) {
            return res.status(404).json({ 
                success: false, 
                error: `找不到桌號 ${tableNumber}` 
            });
        }

        const tableId = tablesDatabase.results[0].id;
        ...
        res.json({ 
            success: true, 
            message: `桌號 ${tableNumber} 狀態已更新為 ${status}` 
        });
...
// 更新訂單狀態（結帳用）
app.post('/api/notion/update-orders-status', async (req, res) => {
    try {
        const { tableNumber, status } = req.body;
        console.log(`🔄 更新訂單狀態 - 桌號: ${tableNumber}, 狀態: ${status}`);

        // 查詢該桌號的所有訂單
        const ordersDatabase = await notion.databases.query({
        ...
        // 更新所有訂單的狀態
        const updatePromises = ordersDatabase.results.map(order => 
            notion.pages.update({
                page_id: order.id,
                properties: {
                    '狀態': {
        ...
        res.json({ 
            success: true, 
            message: `桌號 ${tableNumber} 的 ${ordersDatabase.results.length} 筆訂單狀態已更新為 ${status}` 
        });
...
// 獲取桌況資料
app.get('/api/notion/database/tables', async (req, res) => {
    try {
        console.log('🔍 正在查詢桌況資料庫...');
        ...
        console.error('❌ 查詢桌況資料庫失敗:', error);
        res.status(500).json({ success: false, error: error.message });
    }
});

// 獲取訂單資料
app.get('/api/notion/database/orders', async (req, res) => {
    try {
        console.log('🔍 正在查詢訂單資料庫...');
